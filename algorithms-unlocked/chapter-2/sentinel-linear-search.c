...
int main()
{
  ...
  int index = sentinel_linear_search(arr, DEFAULT_LENGTH, key);

  printf("\nSearching %d in the array ...\n", key);
  printf("[+] %d was founded at index %d.\n", key, index);

  key = generatekey(DEFAULT_LENGTH);
  index = sentinel_linear_search(arr, DEFAULT_LENGTH, key);
